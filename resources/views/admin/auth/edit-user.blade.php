<!-- resources/views/admin/auth/edit-landlord.blade.php -->
@extends('admin.dashboard')

@section('content')
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6">Edit Landlord</h2>
    ...
    <form action="{{ route('admin.landlords.update', $landlord->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" value="{{ old('name', $landlord->name) }}" required>
            @error('name')
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" id="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" value="{{ old('email', $landlord->email) }}" required>
            @error('email')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
